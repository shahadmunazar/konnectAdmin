...
import { Form, Row, Col, Button, Modal } from 'react-bootstrap';
import ImageComponent from './ImageComponent'; // Ensure this displays an image (or use img directly)

const PhotoUploadComponent = ({ value, onChange, error }) => {
    ...
    const retakePhoto = () => setShowCameraModal(true);
    ...
    return (
        <>
                ...
                <Col sm={9}>
                    {value ? (
                        <div className="text-center">
                            ...
                            />
                            <div className="mt-2 d-flex justify-content-center gap-2">
                                <Button size="sm" onClick={retakePhoto}>Retake</Button>
                                <Button size="sm" variant="danger" onClick={removePhoto}>Remove</Button>
                            </div>
                        </div>
                    ) : (
                        <div
            ...
            <Modal show={showCameraModal} onHide={() => setShowCameraModal(false)} centered>
                ...
                <Modal.Body className="text-center">
                    <video ref={videoRef} style={{ width: '100%', maxHeight: '400px', borderRadius: '8px' }} autoPlay />
                    <canvas ref={canvasRef} style={{ display: 'none' }} />
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="secondary" onClick={() => setShowCameraModal(false)}>Cancel</Button>
                    <Button variant="primary" onClick={capturePhoto}>Capture</Button>
                </Modal.Footer>
            </Modal>
        </>
    );
